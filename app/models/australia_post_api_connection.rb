...
class AustraliaPostApiConnection
  ...
  def get_suburbs_for_postcode(postcode)
    request_url = 'postcode/search.json?q='+postcode.to_s+'&state='
    valid_postcodes = api_call(request_url)
  ...
  def api_call(method)
    if method.include? 'postcode/search'
      api_prefix = 'https://digitalapi.auspost.com.au'
    else
      api_prefix = self.api_root
    end
    Thread.abort_on_exception = true
      ...
      command = Thread.new do
        ...
        Thread.current["httparty_response"] = HTTParty.get("#{api_prefix}/#{method}",
